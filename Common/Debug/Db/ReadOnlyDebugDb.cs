...
{
    public void Append<T>(T entry) where T : Tyr.Common.Debug.IEntry
        => throw new InvalidOperationException("Cannot append to a read-only debug database.");

    public IEnumerable<T> Query<T>(string module, Timestamp t0, Timestamp t1, string? shardKey = null, int? maxCount = null) where T : Tyr.Common.Debug.IEntry
        => inner.Query<T>(module, t0, t1, shardKey, maxCount);

    public IEnumerable<T> QueryWithoutMeta<T>(string module, Timestamp t0, Timestamp t1, string? shardKey = null, int? maxCount = null) where T : Tyr.Common.Debug.IEntry
        => inner.QueryWithoutMeta<T>(module, t0, t1, shardKey, maxCount);

    public IEnumerable<T> Query<T>(Timestamp t0, Timestamp t1, string? module = null, int? sourceLocationId = null, string? shardKey = null, int? maxCount = null) where T : Tyr.Common.Debug.IEntry
        => inner.Query<T>(t0, t1, module, sourceLocationId, shardKey, maxCount);

    public IEnumerable<T> QueryAll<T>(Timestamp t0, Timestamp t1, string? shardKey = null, int? maxCount = null) where T : Tyr.Common.Debug.IEntry
        => inner.QueryAll<T>(t0, t1, shardKey, maxCount);

    public IEnumerable<string> QueryModules() => inner.QueryModules();

    public IEnumerable<string> QueryShardKeys<T>(string module) where T : Tyr.Common.Debug.IEntry
        => inner.QueryShardKeys<T>(module);

    public IEnumerable<Meta> QuerySourceLocations<T>(string module) where T : Tyr.Common.Debug.IEntry
        => inner.QuerySourceLocations<T>(module);

    public Meta? TryGetShardMeta<T>(string module, string shardKey) where T : Tyr.Common.Debug.IEntry
