...
public class ChipKickLinearPositionTimeOffsetFitter
{
    ...
    private LinearSolveResult? LinearSolve(List<RawBall> ballRecords, double timeOffset)
    {
        ...
        var matrix = new DenseMatrix(numRecords * 2, 5);
        ...
        for (var i = 0; i < numRecords; i++)
        {
            ...
            matrix.SetRow(i * 2, [cameraPosition.Z, 0.0, cameraPosition.Z * time, 0.0, (measurement.X - cameraPosition.X) * time]);
            matrix.SetRow((i * 2) + 1, [0.0, cameraPosition.Z, 0.0, cameraPosition.Z * time, (measurement.Y - cameraPosition.Y) * time]);
    ...
    private SolvedKick? PostProcess(List<RawBall> ballRecords, LinearSolveResult? bestResult)
    {
        ...
        var solution = bestResult.Solution;
        ...
        _kickPosition = new Vector2((float)solution[0], (float)solution[1]);
        _kickTimestamp = ballRecords[0].CaptureTimestamp - DeltaTime.FromSeconds(bestResult.TimeOffset);
        LastL1Error = ComputeL1Error(ballRecords, _kickPosition, kickVelocity, _kickTimestamp);
        ...
        return new SolvedKick(
            ...
            _kickTimestamp,
            Vector2.Zero,
            nameof(ChipKickLinearPositionTimeOffsetFitter));
    }

    private double ComputeL1Error(
        List<RawBall> ballRecords,
        Vector2 kickPosition,
        Vector3 kickVelocity,
        Timestamp kickTimestamp)
    {
        var error = 0.0;
        var kickPosition3D = kickPosition.Xyz();
