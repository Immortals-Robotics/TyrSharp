...
public static class FieldSizeExtensions
{
    ...
    public static Rectangle ExtendedOwnPenaltyArea(this FieldSize field, float extensionSize = 200.0f)
    {
        float penaltyAreaHalfWidth = field.PenaltyAreaWidth / 2.0f;
        var start = new Vector2(field.OwnGoal().X, -(penaltyAreaHalfWidth + extensionSize));
        float w = -Context.SideSign * (extensionSize + field.PenaltyAreaDepth);
